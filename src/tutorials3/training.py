...
import numpy as np
import torch,csv,random,os
from torch import optim
from torch.nn import functional as F
from torch.optim import lr_scheduler
from tqdm import tqdm
from model import MGC_Model
from function import train_test_mask
...
def mgc_repeat_training(X: torch.Tensor,
    ...
    for i, _ in enumerate(tqdm(range(repeat_num), disable=hide_repeat_tqdm)):
        trained_MGC_model = mgc_training(X, adj, target, display=display_loss,#X 3798*9;adj 10*3798*3798;target 3798*1
...
def mgc_training(X, adj, target_gene_expr, train_mask, test_mask, val_mask, seed=0,
    ...
    MGC_model = MGC_Model(feature_num=cell_type_num,#9
                          hidden_num=hidden_num,#9
                          support_num=lr_pair_num,#10
