...
run_cellchat <- function(count_path, meta_path, output_path){
  ...
  meta = read.table(meta_path,sep = '\t',header = T,stringsAsFactors = F,row.names = 1) 
  barcode = rownames(meta)
  label = as.vector(meta$cell_type)
  names(label) = barcode
...
run_cellchat(count_path, meta_path, output_path)
